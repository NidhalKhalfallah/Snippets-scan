...
int search_employee_age(const char* employeename, const char* password) {
    ...
    if (strcmp(password, "1") == 0) {
        age = search_employee_age_1("SELECT age FROM employees WHERE employeename='%s'", employeename);
    } else {
        age = search_employee_age_2("SELECT age FROM employees WHERE employeename='%s' AND password='%s'", employeename, password);
    }
